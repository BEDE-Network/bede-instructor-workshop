...
By the end of this morning's session you should be able to:

-   Identify the major components of the RStudio interface
-   Create an .Rmd file and successfully `knit` a document that includes text, R code, and output
-   Read in a data set and perform basic data wrangling and visualization procedures on it in R

For this session, you can either work in RStudio on your own computer (R/RStudio install instructions can be found [here](https://www.rstudio.com/products/rstudio/download/#download)) or you can work in an Posit Cloud (formerly known as RStudio Cloud) session. To start a Posit Cloud session go to <https://posit.cloud/> and use your preferred log in method, or create a new account.

# Teaching R with RStudio

In this workshop, we will be working with R through RStudio. R is a programming language (specifically a statistical analysis programming language), while RStudio is an **Integrated Development Environment**, or **IDE**. RStudio provides a user-friendly interface for R (and other programming languages) that help make writing/running code and visualizing data easier.

## RStudio layout

There are four panels in the RStudio interface (though you may only have three open when you first start it), each with useful information:

-   Console / Terminal panel (lower-left)
-   Environment / History / Git (upper-right)
-   Files / Plots / Packages / Help (lower-right)
-   Source / Editor (upper-left; may not appear initially)

# File management

Good file management is essential in data science. It's great if you can instill best practices with your students from the start! 
...
A **working directory** is the directory / folder on your computer that R will use as its reference point (or "home base") when loading & saving files. Your working directory will be specific to *your* computer; e.g., my current working directory is `C:/Users/kfarrell/Desktop/DataSci`.

You can set your working directory by typing a setwd() command in the Console (bottom-left panel) that includes your full directory path.

```{r, setwd, eval=FALSE}

setwd("Users/kfarrell/Desktop/DataSci/")

```

You can also use a Point-and-click method by clicking 'Session' \> 'Set Working Directory' \> 'Choose Directory' and select your `DataSci` folder.

While it is possible to manually set your working directory each time you're working in RStudio, it is not a best practice for reproducibility, because you would need to changed your working directory to run on any other computer (or a collaborator's computer, etc.).

Instead, we will prefer to work in **RStudio Projects**. RStudio Projects encourage good file management by having all related files in the same set of folders. Having your students organize their work using Projects also tends to reduce their frustration in looking for specific data files or scripts on their computer.

When working with RStudio Projects, you never need to set a working directory, because your project will use "relative paths" to find your files *relative to* your project directory folder. This can be especially helpful if you work on more than one computer, or are sharing files with collaborators.

## Create an RStudio Project

Let's set up our `DataSci` directory as an RStudio project. To make a new project, got to the upper right-hand side of the RStudio interface, where it says **Project: (None)**. Click the down arrow, select "New Project", then "Existing Directory" in the pop-up window. Navigate to your `DataSci` directory, then select "Create Project".

# Ways to code in R

## R script files

Often, we write commands using *.R script files*. Script files contain primarily code and brief comments that you include to explain the pieces of code are doing.

> **Important:** within an R file (.R), you use the \# sign to add comments. Anything written after the \# is **not** interpreted when you run the code.

### Try it!

1.  Create a new, blank .R script by clicking 'File' \> 'New file' \> 'R Script'.
2.  From our Google Drive folder, download the practice_data.csv file into your `data` subfolder within your `DataSci` project.
3.  Type in the following code, then run it!

```{r, eval = FALSE}

raw_data <- read.csv("data/practice_data.csv")

plot(y ~ x, data = raw_data)

```

4.  Add a comment at the top of your script to describe what the script does.
5.  Save your file (.R) to the `scripts` subfolder within your `DataSci` Project.

You've now done your first data import, created and run your first script, *and* practiced effective file organization within your RStudio project! Whoo!!

## R Markdown files

[R Markdown](https://rmarkdown.rstudio.com/) allows us to integrate text, R code chunks, and any output from that code into a seamless document that can be easily **reproduced**.

Markdown is a formatting syntax for authoring HTML, PDF, and MS Word documents. While some of the details of using Markdown (and thus R Markdown) can get tricky, the basics are fairly straightforward to pick up. 

Newer versions of RStudio make editing in Markdown nearly seamless, as you can work in "Visual" rather than "Source" mode. Additional integrated tools to learn and use Markdown can be found by going to the `Help` tab and selecting `Markdown Quick Reference`or `Help` \> `Cheatsheets`.

### Getting started with R Markdown

To create a "blank" Markdown document, go to the new file button (upper left corner of the RStudio interface, or `File` \> `New File`) and choose the R Markdown option. A screen will pop up requesting information such as Title, Author, and Output Format. Give your new doc a title and select **HTML** as the output format. A new file that is pre-populated with some text will open in the `Source` panel.
...
When you click the **Knit** button, a document will be generated that includes your text, R script chunks, and the output of any embedded code chunks within the document. Click **Knit** now to see what happens.

### How to structure an R Markdown file

The following are some recommended best practices for structuring an .Rmd file:

-   Name, title, and a description of the file purpose at the top
-   First code chunk should read in all necessary packages and data files (keep it all together here at the top!)
-   Each chunk represents one "unit" of code. Typically, you will have lots of chunks in your file, with each chunk representing a substantial task in your script.
-   Make good use of text, notes, and headers in the white space between the chunks. This is meant to be the "human readable" part of your document and contextualize what you're doing.
-   Knit often to make sure things work and that your code is free of bugs! If one (or more) of your chunks has an error, your document will not knit... so troubleshoot, then knit again!
...
After importing a data set, we want to take a quick look at the data. There are a number of ways we can do this!
...
```{r Summarize data set}

summary(mammals)

```

Finally, we might want to examine specific columns; for example, to see how many unique sites or unique species the data set includes.
...
We can select specific *columns* from our data set to retain (or remove) using `select()` commands. When used on its own, the select command requires that we first name the data object we're selecting from within, then indicate the focal columns.
...
Similarly, we can use a `filter()` to retain (or remove) *rows* from our data set based on criteria we specify.
...
This means that there is a mismatch between how many "bins" along the x-axis you are setting up, relative to the size of the data set.
...
You may also have noticed another warning: "Removed 2 rows containing non-finite values (`stat_bin()`)." This is just R giving you a heads-up that two observations in your data set were missing values for the requested x-axis variable (in this case, bill length), so those observations were not plotted.

It is important when making your own figures (& training your students!), that you investigate whether this type of warning is driven by NA values in your data set (no problem) *or* if you have manually set axis limits that are excluding data points (adjust your axes to encompass your observations!).

Now that the bin number is a better fit to our data set, we can add additional aesthetics and ggplot layers to customize our plot and the story it is telling.
...
While boxplots pack in a lot of information, it can still be useful to include a visualization of the full data set. We can use an additional geom, like `geom_jitter()` to include the raw data points.

We'll include this geom *before* our `geom_boxplot()` call so that the points appear *behind* the boxplots (layers are added to our ggplot in the order they are written). Note that we'll now specify `fill = NA` within our `geom_boxplot()` call so that the boxes themselves are transparent, allowing us to see all the jittered points.
...
We use linear regression to quantify the slope of best fit line between two continuous variables and test whether that slope is significantly different from zero. Our regression model will also tell us about how strong a predictive relationship is between our two variables.
